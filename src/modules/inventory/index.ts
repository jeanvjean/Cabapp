...
interface NewProductInterface{
  itemDescription: ProductInterface['itemDescription'];
  equipmentModel: ProductInterface['equipmentType'];
  equipmentType: ProductInterface['equipmentModel'];
  areaOfSpecialization: ProductInterface['areaOfSpecialization'];
  ...
  reorderLevel: ProductInterface['reorderLevel'];
  location?: ProductInterface['location'];
  referer?: ProductInterface['referer'];
...
class Product extends Module {
  ...
  public async createBranch(data: NewBranchInterface): Promise<BranchInterface|undefined> {
    try {
      // @ts-ignore
      const checkEmail = await this.user.findOne({email: data.branchAdmin});
      if (checkEmail) {
  ...
  public async createProduct(data: NewProductInterface, user: UserInterface): Promise<ProductInterface|undefined> {
    try {
      ...
      const findProduct = await this.product.findOne({partNumber, branch: user.branch});
      if (findProduct) {
        throw new BadInputFormatException('a product with this part number already exists in your branch');
      }

      const product = new this.product({...data, branch: user.branch});

      const findP = await this.product.find({}).sort({serialNumber: -1}).limit(1);
      let sn;
      if (findP.length > 0) {
        // @ts-ignore
  ...
  public async createSupplier(data: NewSupplierInterface, user: UserInterface): Promise<SupplierInterface|undefined> {
    try {
      const suppliers = await this.supplier.find({}).sort({gen_id_no: -1}).limit(1);
      let uid;
      if (!suppliers[0]) {
        uid = 1;
      } else {
  ...
  public async fetchSuppliers(query: QueryInterface, user: UserInterface): Promise<SupplierInterface[]|undefined> {
    try {
      ...
      // @ts-ignore
      const suppliers= await this.supplier.paginate(q, options);
  ...
  public async addInventory(data: NewInventory, user: UserInterface): Promise<InventoryInterface|undefined> {
    try {
      ...
      const hod = await this.user.find({role: user.role, subrole: 'head of department'});
  ...
  public async approveGrn(data: ApproveGrnInput, user: UserInterface): Promise<InventoryInterface|undefined> {
    try {
      const grn = await this.inventory.findById(data.grnId);

      if (grn) {
        if (data.status == 'rejected') {
          const initiator = await this.user.findById(grn.inspectingOfficer);
          await new Notify().push({
            subject: 'GRN approval',
        ...
        if (grn.direction == productDirection.IN) {
          for (const product of products) {
            const prod = await this.product.findOne({partNumber: product.partNumber, branch: user.branch});
          // @ts-ignore
          ...
          for (const product of products) {
            const prod = await this.product.findOne({asnlNumber: product.partNumber, branch: user.branch});
          // @ts-ignore
  ...
  public async disburseProduct(data: NewDisburseInterface, user: UserInterface): Promise<DisburseProductInterface|undefined> {
    try {
      ...
      const apUser = await this.user.findById(disbursement.nextApprovalOfficer);
      await new Notify().push({
  ...
  public async approveDisbursment(data: ApprovalInput, user: UserInterface): Promise<DisburseProductInterface|undefined> {
    try {
      const loginUser = await this.user.findById(user._id).select('+password');
      const matchPWD = await loginUser?.comparePWD(data.password, user.password);
      if (!matchPWD) {
      ...
      if (data.status == ApprovalStatus.REJECTED) {
        if (disbursement?.approvalStage == stagesOfApproval.STAGE1) {
          const AO = disbursement.approvalOfficers.filter((officer)=> officer.stageOfApproval == stagesOfApproval.STAGE1);
          const track = {
            title: 'Corrections',
        ...
        const newBranchApprovalOfficer = await this.user.findOne({branch: data.fromBranch, subrole: 'sales executive', role: 'sales'});
        ...
        if (disbursement?.approvalStage == stagesOfApproval.START) {
          ...
          const apUser = await this.user.findById(disbursement.initiator);
          await new Notify().push({
            subject: 'Product disbursal',
          ...
          const brenchRequestApproval = await this.user.findOne({branch: user.branch, subrole: 'head of department'}).populate({
            path: 'branch', model: 'branches'
          });
          // console.log(brenchRequestApproval)
          const branchAdmin = await this.user.findOne({branch: brenchRequestApproval?.branch, subrole: 'superadmin'});
          const track = {
            title: 'Approval Prorcess',
            stage: stagesOfApproval.STAGE2,
            status: ApprovalStatus.APPROVED,
          ...
          // set next branch
          const nb = await this.user.findById(newBranchApprovalOfficer?._id);
          // @ts-ignore
  ...
  public async fetchDisburseRequests(query: QueryInterface, user: UserInterface): Promise<DisbursePoolResponse|undefined> {
    try {
      ...
      const totalApproved = await this.disburse.find({branch: user.branch, disburseStatus: TransferStatus.COMPLETED});
      const totalPending = await this.disburse.find({branch: user.branch, disburseStatus: TransferStatus.PENDING});
      return Promise.resolve({
  ...
  public async fetchProductRequests(query: QueryInterface, user: UserInterface): Promise<DisbursePoolResponse|undefined> {
    try {
      ...
      const totalApproved = await this.disburse.find({branch: user.branch, requestApproval: TransferStatus.COMPLETED});
      const totalPending = await this.disburse.find({branch: user.branch, requestApproval: TransferStatus.PENDING});
      return Promise.resolve({
        disburse: disbursements,
        count: {
          totalApproved: totalApproved.length | 0,
