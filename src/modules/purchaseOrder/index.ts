...
class PurchaseOrder extends Module {
    ...
    public async createPurchaseOrder(data: newPurchaseOrder, user: UserInterface): Promise<PurchaseOrderInterface|undefined> {
      try {
        ...
        const ex = await this.purchase.find({}).sort({initNum: -1}).limit(1);
        let on;
        if (ex[0]) {
          on = ex[0].initNum + 1;
        } else {
        ...
        const orderNumber = padLeft(on, 6, '');
        ...
        const fEcr = await this.ecr.findById(purchase.ecr);
        if (!fEcr) {
          throw new BadInputFormatException('ecr id not found');
        }

        const remain = [];
        for (const cyl of purchase.cylinders) {
          const cylinder = await this.cylinder.findOne({cylinderNumber: cyl.cylinderNo});
          if (!cylinder) {
        ...
        const approvalUser = await this.user.findById(purchase.nextApprovalOfficer);
    ...
    public async fetchPurchaseOrders(query: QueryInterface, user: UserInterface): Promise<purchaseOrderPool|undefined> {
      try {
        const {page, limit, search, fromBranch, branch, approvalStatus, supplier, fromDate, toDate} = query;
        const options = {
          page: page || 1,
          limit: limit || 10,
          populate: [
      ...
      try {
        ...
        if (data.status == ApprovalStatus.REJECTED) {
          if (purchase?.approvalStage == stagesOfApproval.STAGE1) {
            const AO = purchase.approvalOfficers.filter((officer)=>officer.stageOfApproval == stagesOfApproval.STAGE1);

            const checkOfficer = purchase.approvalOfficers.filter((officer)=> `${officer.id}` == `${user._id}`);
            ...
            const AO = purchase.approvalOfficers.filter((officer)=>officer.stageOfApproval == stagesOfApproval.STAGE2);
          ...
          if (purchase?.approvalStage == stagesOfApproval.START) {
            ...
            const approvalUser = await this.user.findById(purchase.initiator);
            await new Notify().push({
              subject: 'Purchase Order',
