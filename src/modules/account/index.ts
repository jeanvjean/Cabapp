...
class Account extends Module {
    ...
    public async createReciept(data: newRecieptInterface, user: UserInterface): Promise<RecieptInterface|undefined> {
      try {
        ...
        const exists = await this.account.find({}).sort({invInit: -1}).limit(1);
        let sn;
        if (exists[0]) {
        ...
        const init = 'INV';
        const invoiceNumber = padLeft(sn, 6, '');
        reciept.invoiceNo = init+invoiceNumber;
        reciept.invInit = sn;

        const sales = await this.salesRequisition.findById(reciept.salesReq);
        if (sales) {
    ...
    public async updateInvoice(data: invoiceUpdateInput): Promise<invoiceResponse|undefined> {
      try {
        ...
        const message = invoice?.outstandingBalance <= 0 ? 'Paid out' : 'payment updated';
