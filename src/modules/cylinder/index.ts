...
class Cylinder extends Module {
  ...
  public async createCylinder(data: NewCylinderInterface, user: UserInterface): Promise<CylinderInterface|undefined> {
    try {
      const exist = await this.cylinder.findOne({colorCode: data.colorCode});
      if (exist) {
      ...
      const newGas = await this.cylinder.create(payload);
      await createLog({
        user: user._id,
        activities: {
          title: 'Cylinder type',
  ...
  public async registerMultipleCylinders(file: object, user: UserInterface): Promise<any> {
    try {
      ...
      // @ts-ignore
      const filepath = await files.mv(path + '/' + files.name);
  ...
  public async fetchCylinders(query: QueryInterface): Promise<FetchCylinderInterface|undefined> {
    try {
      // @ts-ignore
      const cylinders = await this.cylinder.find({}, {...query}).sort({createdAt: -1});
  ...
  public async regCylinder(data: NewCylinderRegisterationInterface, user: UserInterface): Promise<RegisteredCylinderInterface|undefined> {
    try {
      ...
      const manDate = new Date(data.dateManufactured);
      const gName = await this.cylinder.findById(data.gasType);
      if (!gName) {
        throw new BadInputFormatException('select a valid gasType');
      }
      // console.log(gName, data);
      const cylCount = await this.registerCylinder.find({}).sort({cylNo: -1}).limit(1);
      let barcode;
      if (cylCount[0]) {
      ...
      const newRegistration = await this.registerCylinder.create(payload);
      await createLog({
        user: user._id,
        activities: {
  ...
  public async updateRegCylinder(data: UpdateRegisteredCylinder, user: UserInterface): Promise<RegisteredCylinderInterface|undefined> {
    try {
      ...
      const updatedCyliner = await this.registerCylinder.findByIdAndUpdate(
        cylinder._id,
        {
          $set: data
        },
  ...
  public async approveCylinderChange(data: CylinderChangeApproval, user: UserInterface): Promise<ChangeCylinderInterface|undefined> {
    try {
      ...
      const change = await this.change_gas.findById(data.changeId).populate(
        [
      ...
      if (data.status == ApprovalStatus.REJECTED) {
        if (change?.approvalStage == stagesOfApproval.STAGE1) {
          const AO = change.approvalOfficers.filter((officer)=>officer.stageOfApproval == stagesOfApproval.STAGE1);
          const track = {
          ...
          const checkOfficer = change.approvalOfficers.filter((officer)=> `${officer.id}` == `${user._id}`);
          ...
          const apUser = await this.user.findById(change.nextApprovalOfficer);
          await new Notify().push({
          ...
          const AO = change.approvalOfficers.filter((officer)=>officer.stageOfApproval == stagesOfApproval.STAGE2);
          const track = {
            title: 'Approval Process',
        ...
        if (change?.approvalStage == stagesOfApproval.START) {
          ...
          const branchAdmin = await this.user.findOne({branch: user.branch, subrole: 'superadmin'});
          // @ts-ignore
          ...
          const apUser = await this.user.findById(change.initiator);
          await new Notify().push({
            subject: 'New Cylinder Change',
            content: `A Cylinder Change you initiated has been approved to view ${env.FRONTEND_URL}/fetch-condemn-details/${change._id}`,
            user: apUser
          });
          const cylinders = change.cylinders;
          for (const cyl of cylinders) {
            const changeCyl = await this.registerCylinder.findById(cyl);
            // @ts-ignore
            changeCyl?.gasType = change.gasType;
            const ngName = await this.cylinder.findById(change.gasType);
            // @ts-ignore
  ...
  public async fetchChangeCylinderRequests(query: QueryInterface, user: UserInterface): Promise<ChangeCylinderInterface[]|undefined> {
    try {
      ...
      // @ts-ignore
      const changes = await this.change_gas.paginate(q, options);
      return changes;
    } catch (e) {
      this.handleException(e);
    }
  ...
  public async fetchRegisteredCylinders(query: QueryInterface, user: UserInterface): Promise<RegisteredCylinderPoolInterface|undefined> {
    try {
      ...
      // @ts-ignore
      const registeredCylinders = await this.registerCylinder.paginate(q, options);
  ...
  public async fetchCylinderWithScan(data: CylinderDetailsInterface, user: UserInterface): Promise<any> {
    try {
      const {barcode, cylinderNumber, assignedNumber} = data;
      ...
      const cylinder = await this.registerCylinder.findOne(q).populate([
        {path: 'assignedTo', model: 'customer'},
        {path: 'branch', model: 'branches'},
        {path: 'gasType', model: 'cylinder'},
        {path: 'toBranch', model: 'branches'}
      ...
      const lastOcn = await this.ocn.find({
      ...
      if (lastOcn[0]) {
        ...
        const lastDn = await this.waybill.findOne({
          ocn: lastOcn[0]._id
        });
  ...
  public async fetchFaultyCylinders(query: QueryInterface, user: UserInterface): Promise<RegisteredCylinderInterface[]|undefined> {
    try {
      ...
      // @ts-ignore
      const cylinders = await this.registerCylinder.paginate(q, options);
      return Promise.resolve(cylinders);
    } catch (e) {
      this.handleException(e);
    }
  ...
  public async approveCondemnation(data: ApproveCondemn, user: UserInterface): Promise<CondemnCylinderInterface|undefined> {
    try {
      ...
      const condem = await this.condemn.findById(data.condemnId).populate(
        [
          {path: 'initiator', model: 'User'}
        ]
      );
      ...
      if (data.status == ApprovalStatus.REJECTED) {
        if (condem?.approvalStage == stagesOfApproval.STAGE1) {
          const AO = condem.approvalOfficers.filter((officer)=>officer.stageOfApproval == stagesOfApproval.STAGE1);
          const track = {
            title: 'Approval Process',
            stage: stagesOfApproval.STAGE2,
            status: ApprovalStatus.REJECTED,
          ...
          const checkOfficer = condem.approvalOfficers.filter((officer)=> `${officer.id}` == `${user._id}`);
          if (checkOfficer.length == 0) {
          ...
          const apUser = await this.user.findById(condem.nextApprovalOfficer);
          await new Notify().push({
          ...
          const AO = condem.approvalOfficers.filter((officer)=>officer.stageOfApproval == stagesOfApproval.STAGE2);
          const track = {
            title: 'Approval Process',
            stage: stagesOfApproval.STAGE3,
            status: ApprovalStatus.REJECTED,
        ...
        if (condem?.approvalStage == stagesOfApproval.START) {
          ...
          const apUser = await this.user.findById(condem.initiator);
          await new Notify().push({
          ...
          const cylinders = condem.cylinders;
  ...
  public async fetchArchivedCylinder(query: QueryInterface, user: UserInterface): Promise<ArchivedCylinder[]|undefined> {
    try {
      ...
      // @ts-ignore
      const cylinders = await this.archive.paginate(q, options);
      return Promise.resolve(cylinders);
    } catch (e) {
      this.handleException(e);
    }
  ...
  public async transferCylinders(data: TransferCylinderInput, user: UserInterface): Promise<ApprovalResponse|undefined> {
    try {
      ...
      for (let i = 0; i < data.cylinders.length; i++) {
        const cyl = data.cylinders[i];
        const fCyl = await this.registerCylinder.findById(cyl);
        if (fCyl) {
          if (!fCyl.available) {
            pulled.push(cyl);
            send.push({
      ...
      const transfer = new this.transfer({...data, branch: user.branch});
      ...
      const message = pulled.length > 0 ? `some cylinders in the request may have been assigned to another customer cylinders:${send}` : `Transfer Initiated`;
      await transfer.save();
      await createLog({
        user: user._id,
        activities: {
  ...
  public async approveTransfer(data: ApprovalInput, user: UserInterface): Promise<ApprovalResponse|undefined> {
    try {
      ...
      const transfer = await this.transfer.findById(data.id).populate(
        [
          {path: 'initiator', model: 'User'}
        ]
      );
      ...
      if (data.status == ApprovalStatus.REJECTED) {
        if (transfer?.approvalStage == stagesOfApproval.STAGE1) {
          const AO = transfer.approvalOfficers.filter((officer)=>officer.stageOfApproval == stagesOfApproval.STAGE1);
          const track = {
            title: 'Approval Process',
            stage: stagesOfApproval.STAGE2,
            status: ApprovalStatus.REJECTED,
          ...
          const checkOfficer = transfer.approvalOfficers.filter((officer)=> `${officer.id}` == `${user._id}`);
          if (checkOfficer.length == 0) {
            transfer.approvalOfficers.push({
              name: user.name,
              id: user._id,
          ...
          for (let i = 0; i < transfer.cylinders.length; i++) {
            const cyl = transfer.cylinders[i];
            const fCyl = await this.registerCylinder.findById(cyl);
            if (fCyl) {
              if (!fCyl?.available) {
                // @ts-ignore
          ...
          const apUser = await this.user.findById(transfer.nextApprovalOfficer);
          await new Notify().push({
            subject: 'New cylinder transfer',
          ...
          const AO = transfer.approvalOfficers.filter((officer)=>officer.stageOfApproval == stagesOfApproval.STAGE2);
          const track = {
            title: 'Approval Process',
            stage: stagesOfApproval.STAGE3,
            status: ApprovalStatus.REJECTED,
        ...
        if (transfer?.approvalStage == stagesOfApproval.START) {
          ...
          const message = pulled.length > 0 ? `some cylinders in the request may have been assigned to another customer cylinders:${send}` : `Approved`;
          ...
          if (transfer.type == TransferType.TEMPORARY) {
            for (const cylinder of cylinders) {
              ...
              if (cyl) {
                ...
                const date = new Date(cyl.holdingTime);
                schedule.scheduleJob(
                  new Date(date),
                  async function(id: string) {
                    const holdingCylinder = await RegisteredCylinder.findById(id);
                    if (holdingCylinder) {
  ...
  public async faultyCylinder(cylinderId: string, user: UserInterface): Promise<RegisteredCylinderInterface|undefined> {
    try {
      ...
      const apUser = await this.user.findOne({role: 'production', subrole: 'head of department', branch: cylinder.branch});
      await new Notify().push({
  ...
  public async fixedFaultyCylinder(cylinderId: string, user: UserInterface): Promise<RegisteredCylinderInterface|undefined> {
    try {
      ...
      const apUser = await this.user.findOne({role: 'sales', subrole: 'head of department', branch: cylinder.branch});
      await new Notify().push({
        subject: 'Faulty cylinder',
  ...
  public async fetchTransferRequets(query: QueryInterface, user: UserInterface): Promise<TransferRequestPool|undefined> {
    try {
      ...
      // @ts-ignore
      const transfers = await this.transfer.paginate(q, options);
      const transferReq= await this.transfer.find({branch: user.branch});
      const totalApproved = transferReq.filter(
        // @ts-ignore
        (transfer)=>transfer.transferStatus == TransferStatus.COMPLETED
      );
      const totalPending = transferReq.filter(
        // @ts-ignore
  ...
  public async fetchChangeCylinderRequest(query: QueryInterface, user: UserInterface): Promise<TransferCylinder|undefined> {
    try {
      // @ts-ignore
      const changeRequests = await this.transfer.paginate({branch: user.branch, type: TransferType.CHANGEGAS}, {...query, sort: {createdAt: -1}});
