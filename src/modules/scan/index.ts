...
class Scan extends Module {
    ...
    public async ScanCylinder(data: ScanInput): Promise<ScanInterface | undefined> {
      try {
        ...
        const found = await this.scan.findOne({formId: data.formId});
        if (!found) {
          throw new BadInputFormatException('no form with this formId found');
        }
        // console.log(data.barcode);
        const cyl = await this.cylinder.findOne({barcode: data.barcode});
        // console.log(cyl)
        if (!cyl) {
          throw new BadInputFormatException('cylinder info not found');
        }
        const m = found.cylinders.map((doc)=> doc.barcode);
    ...
    public async updateCyliderScan(data: UpdateScan): Promise<ScanInterface|undefined> {
      try {
        const {cylinders, formId} = data;
        const scan = await this.scan.findOne({formId});
        if (!scan) {
          throw new BadInputFormatException('form not found');
        }
        const saveCyl = scan.cylinders;
        for (const cylinder of cylinders) {
          const m = scan.cylinders.map((doc)=> doc.barcode);
    ...
    public async fetchScans(query: QueryInterface): Promise<ScanInterface[]|undefined> {
      try {
        const {search, page, limit, cylinderNumber, assignedNumber, barcode, status} = query;
        const options = {
          page: page || 1,
          limit: limit || 10,
          sort: {createdAt: -1}
        ...
        // @ts-ignore
        const scan = await this.scan.paginate(q, options);
        return Promise.resolve(scan);
      } catch (e) {
        this.handleException(e);
      }
    ...
    public async initiateScan(): Promise<ScanInterface|undefined> {
      try {
        const form = new this.scan();
        const oldScan = await this.scan.find({}).sort({initNum: -1}).limit(1);
        let formId;
        if (oldScan[0]) {
