...
class NotificationModule extends Module {
	...
	public async saveMessageToFirebase(payload: WebPush) {
	  if (payload.user) {
	    try {
	      ...
	      const time = Date.now();
	      const rad = await dbRef.child(payload.user._id.toString());
	      const notRef = await rad.child('notification')
	        .push({
	          title: payload.subject,
	          body: payload.content,
	          date: time
	        });
		    	    const red = await rad.child('newNotifications').transaction((counter: number) => (counter || 0) + 1);
	    } catch (error) {
	      console.log(error);
	    }
	  }
	...
	public async saveFormToFirebase(payload: FormData) {
	  if (payload.formId) {
	    try {
	      ...
	      // const time = Date.now()
	      const rad = await formRef.child('form')
	        .set({
	...
	public async sendSms(payload: smsInterface) {
	  try {
	    ...
	    const response = await axios.post(`${SecretKeys.INFOBIP_URL}sms/2/text/advanced`, messagePayload,
	      {
	        headers: {
	          'Content-Type': 'application/json',
