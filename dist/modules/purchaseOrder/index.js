    ...
    createPurchaseOrder(data, user) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                let fEcr = yield this.ecr.findById(purchase.ecr);
                if (!fEcr) {
                    throw new exceptions_1.BadInputFormatException('ecr id not found');
                }
                let remain = [];
                for (let cyl of purchase.cylinders) {
                    let cylinder = yield this.cylinder.findOne({ cylinderNumber: cyl.cylinderNo });
                    if (!cylinder) {
                        throw new exceptions_1.BadInputFormatException(`cylinder with number ${cyl.cylinderNo} does not seem to be found`);
                    }
                    if (!fEcr.cylinders.includes(cylinder._id)) {
                        throw new exceptions_1.BadInputFormatException(`cylinder with number ${cyl.cylinderNo} does not seem to be found on the ECR`);
                    }
                    if (fEcr.cylinders.includes(cylinder._id)) {
