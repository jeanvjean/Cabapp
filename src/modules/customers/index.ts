...
class Customer extends Module {
  ...
  public async createCustomer(data: newCustomerInterface, user: UserInterface): Promise<CustomerInterface|undefined> {
    try {
      ...
      const gen_id = padLeft(cid, 6, '');
      const ucid = 'CUS/'+ gen_id;
  ...
  public async createOrder(data: CreateOrderInterface, user: UserInterface): Promise<OrderInterface|undefined> {
    try {
      ...
      const findOrder = await this.order.find({}).sort({initOn: -1}).limit(1);
      ...
      if (complaint?.complaintType == 'cylinder') {
        if (data.status == ApprovalStatus.REJECTED) {
          if (complaint?.approvalStage == stagesOfApproval.STAGE1) {
            const AO = complaint.approvalOfficers.filter((officer)=>officer.stageOfApproval == stagesOfApproval.STAGE1);
            const track = {
            ...
            const checkOfficer = complaint.approvalOfficers.filter((officer)=> `${officer.id}` == `${user._id}`);
            ...
            const approvalUser = await this.user.findById(AO[0].id);
            new Notify().push({
              subject: 'Complaint',
              content: `A complaint requires your attention click to view ${env.FRONTEND_URL}/customer/fetch-complaints/${complaint._id}`,
              user: approvalUser
            ...
            const AO = complaint.approvalOfficers.filter((officer)=>officer.stageOfApproval == stagesOfApproval.STAGE2);
            const track = {
              title: 'Approval Process',
          ...
          if (complaint?.approvalStage == stagesOfApproval.START) {
            ...
            const approvalUser = await this.user.findById(complaint.nextApprovalOfficer);
            new Notify().push({
              subject: 'Complaint',
              content: `A complaint requires your attention click to view ${env.FRONTEND_URL}/customer/fetch-complaints/${complaint._id}`,
              user: approvalUser
            ...
            const approvalUser = await this.user.findById(complaint.initiator);
            new Notify().push({
              subject: 'Complaint',
  ...
  public async fetchApprovedComplaints(query: QueryInterface, user: UserInterface): Promise<ComplaintInterface[]|undefined> {
    try {
      ...
      for (const comp of complaints.docs) {
        const branch = await this.branch.findById(comp.branch);
        comp.branch = branch;
        const initiator = await this.user.findById(comp.initiator);
        comp.initiator = initiator;
        const nextApprovalOfficer = await this.user.findById(comp.nextApprovalOfficer);
        comp.nextApprovalOfficer = nextApprovalOfficer;
        const customer = await this.customer.findById(comp.customer);
  ...
  public async registerWalkinCustomers(data: newWalkinCustomer, user: UserInterface): Promise<WalkinCustomerInterface|undefined> {
    try {
      ...
      const init = 'ECR';
      const num = padLeft(customer.serialNo, 6, '');
      // @ts-ignore
  ...
  public async fetchWalkinCustomers(query: QueryInterface, user: UserInterface): Promise<WalkinCustomerInterface[]|undefined> {
    try {
      ...
      for (const cust of customers.docs) {
        const branch = await this.branch.findById(cust.branch);
        cust.branch = branch;
        const security = await this.user.findById(cust.security);
        cust.security = security;
        const recievedBy = await this.user.findById(cust.recievedBy);
  ...
  public async updateWalkinCustomer(customerId: string, data: newWalkinCustomer, user: UserInterface): Promise<WalkinCustomerInterface|undefined> {
    try {
      ...
      const updatedCustomer = await this.walkin.findByIdAndUpdate(customer._id, {...data, recievedBy: user._id}, {new: true});
  ...
  public async fetchFilledCustomerCylinders(query: QueryInterface, user: UserInterface): Promise<WalkinCustomerInterface[]|undefined> {
    try {
      ...
      for (const cyl of cylinders.docs) {
        const branch = await this.cylinder.findById(cyl.branch);
        cyl.branch = branch;
      }
      return cylinders;
    } catch (e) {
  ...
  public async customerOrderHistory(query: QueryInterface, user: UserInterface): Promise<any> {
    try {
      const {search, customer_unique_id, activity} = query;
      let q = {
        branch: user.branch
      };
      const or = [];
      ...
      const orders = await this.pickup.find(q);
      if (orders.length <= 0) {
        throw new BadInputFormatException('no order matches the parameters');
      }
      // @ts-ignore
      const mappedCustomer = orders.map((doc)=>{
        return doc.customers;
      });
      // @ts-ignore
      const customerOrder=[];
      for (const ar of mappedCustomer) {
        for (const a of ar) {
